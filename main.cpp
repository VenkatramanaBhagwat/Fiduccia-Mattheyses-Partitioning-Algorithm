...
int initcutSz; // initial cut size
int mincutSz;// final minimum cut size 
int totalArea; // sum of all cell areas
vector<int> sheap;//create maxheap to contain max cell area

void readCellArea(string);// reac each cell area and put it in cll object
void createPartition();// create the patition and update each cell object
void readhgrFile(string); //  read .hgr file
int computeGain(string); // compute each cell gain
int FS(string); // calculate FS(x)
int TS(string); //  calculate TE(x)
...
bool checkAreaConstraint();// check area balance criteria 
